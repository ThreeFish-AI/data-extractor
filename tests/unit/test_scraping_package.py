...
class TestScrapingBackwardCompatibility:
    """验证旧路径 shim 文件的向后兼容性。"""

    def test_shim_scraper_module(self):
        from negentropy.perceives.scraper import WebScraper as ShimWebScraper
        from negentropy.perceives.scraping import WebScraper as CanonicalWebScraper

        assert ShimWebScraper is CanonicalWebScraper

    def test_shim_anti_detection_module(self):
        from negentropy.perceives.anti_detection import (
            AntiDetectionScraper as ShimADS,
        )
        from negentropy.perceives.scraping import (
            AntiDetectionScraper as CanonicalADS,
        )

        assert ShimADS is CanonicalADS

    def test_shim_form_handler_module(self):
        from negentropy.perceives.form_handler import FormHandler as ShimFH
        from negentropy.perceives.scraping import FormHandler as CanonicalFH

        assert ShimFH is CanonicalFH

    def test_shim_browser_utils_module(self):
        from negentropy.perceives.browser_utils import (
            build_chrome_options as shim_bco,
        )
        from negentropy.perceives.scraping import (
            build_chrome_options as canonical_bco,
        )

        assert shim_bco is canonical_bco

    def test_all_scraping_exports_in_dunder_all(self):
        import negentropy.perceives.scraping as pkg
