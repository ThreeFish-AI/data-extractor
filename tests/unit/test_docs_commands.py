...
验证命令文档的结构完整性、链接有效性、环境变量准确性和冗余守护。
"""
...
COMMANDS_DOC = DOCS_DIR / "5-Commands.md"


@pytest.fixture(scope="module")
def doc_content() -> str:
    """读取命令文档内容。"""
    return COMMANDS_DOC.read_text(encoding="utf-8")
...
class TestDocExists:
    ...
    def test_commands_doc_exists(self):
        """5-Commands.md 文件存在。"""
        assert COMMANDS_DOC.exists(), f"{COMMANDS_DOC} 不存在"
