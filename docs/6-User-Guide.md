...
sidebar_position: 6
title: User Guide
description: Document Reader MCP Server 终端用户使用指南，涵盖 MCP Server 部署配置、14 个 MCP 工具参考、API 编程接口及高级使用场景。
last_update:
  author: Aurelius
  date: 2026-03-22
tags:
...
Document Reader 是一个基于 FastMCP 和 Scrapy、markdownify、pypdf、pymupdf 联合构建的强大、稳定的网页内容、PDF 内容提取 MCP Server，具备转换 Web Page、PDF Document 为 Markdown 的能力，专为商业环境中的长期使用而设计。
...
```bash
uvx --with git+https://github.com/ThreeFish-AI/data-extractor.git@v0.1.6 document-reader
```
...
| 变量名                             | 默认值      | 说明                     |
| ---------------------------------- | ----------- | ------------------------ |
| `DATA_EXTRACTOR_TRANSPORT_MODE`    | `http`      | 传输模式：stdio/http/sse |
| `DATA_EXTRACTOR_HTTP_HOST`         | `localhost` | HTTP 服务器绑定地址      |
| `DATA_EXTRACTOR_HTTP_PORT`         | `8081`      | HTTP 端口号              |
...
echo "DATA_EXTRACTOR_HTTP_PORT=8081" >> .env
document-reader
```
...
async with DocumentReaderClient("http://localhost:8081/mcp") as client:
...
### Document Reader Python SDK 使用方式
...
async with DocumentReaderClient(
...
# 更换端口
DATA_EXTRACTOR_HTTP_PORT=8082 document-reader
```
