...
除直接调用 14 个业务工具外，项目还提供一个验证编排层 [`packages/mcp_hub/README.md`](../packages/mcp_hub/README.md)，用于把“输入准备 -> 工具执行 -> trace 复盘 -> 人工验收”收敛为统一工作流。

`mcp_hub` 的主要职责：

- 挂载现有 `data-extractor` MCP 工具
- 提供单工具验证与内置场景验证
...
- Viewer：`http://localhost:8091/viewer`

核心验证工具：

| 工具 | 用途 |
| --- | --- |
| `list_validation_scenarios` | 列出内置验证场景 |
| `run_single_tool_validation` | 对单个 MCP 工具执行一次验证 |
| `run_validation_scenario` | 运行内置 URL / PDF / 链路场景 |
| `list_validation_runs` | 查看最近验证运行 |
| `get_validation_run` | 读取单次运行详情 |
| `get_validation_trace` | 读取单次运行的 trace |
| `get_validation_artifacts` | 读取单次运行的产物清单 |
| `compare_validation_runs` | 对比两次运行摘要 |
| `record_manual_verdict` | 写入人工验收结论 |

`mcp_hub` 当前内置三个场景：

- `complex_webpage_markdown`
...
sequenceDiagram
    ...
    participant T as Data Extractor Tool
    participant S as Run Store
    participant V as Viewer

    U->>H: run_validation_scenario / run_single_tool_validation
    H->>T: 调用目标 MCP Tool
    T-->>H: 返回业务结果
    H->>S: 写入 run / trace / artifact
    U->>V: 打开 /viewer
    V->>S: 读取运行记录与产物
    U->>H: record_manual_verdict
```

人工检查时，建议把关注点分成四层：

1. **输入层**：URL / PDF / 参数是否符合预期
...
### 6. 使用 MCP Hub 验证复杂 URL

适用场景：

- 正文区域复杂、导航噪音多的页面
- 希望人工确认 Markdown 提取效果
- 需要同时检查标题、正文、链接或结构化数据质量

推荐场景：`complex_webpage_markdown`
...
async with client:
    run = await client.call_tool("run_validation_scenario", {
        "scenario_id": "complex_webpage_markdown",
        "inputs": {
...
1. 在 `/viewer` 首页确认出现新的 run
2. 打开 run 详情页，检查 `page_info` 与 `markdown` 两个步骤是否成功
3. 在 `Trace` 区域检查抓取、转换、格式化事件是否完整
4. 在 `Artifacts` 区域查看 JSON 与 Markdown 产物
5. 根据以下清单写入 verdict：
   - 标题与元数据是否合理
...
```python
async with client:
    run = await client.call_tool("run_single_tool_validation", {
        ...
        "label": "复杂网页 Markdown 精查"
    })
```

### 7. 使用 MCP Hub 验证复杂 PDF

适用场景：

- 包含图片、表格、公式的复杂 PDF
- 需要验证页数、元数据、增强提取和 Markdown 结构
- 需要对比不同 PDF 的输出质量

推荐场景：`complex_pdf_conversion`
...
async with client:
    run = await client.call_tool("run_validation_scenario", {
        "scenario_id": "complex_pdf_conversion",
        "inputs": {
            "pdf_source": "/absolute/path/to/complex-paper.pdf"
        }
    })
```
...
1. `page_count`、`word_count`、`metadata` 是否符合文档事实
2. `enhanced_assets` 中图片、表格、公式统计是否合理
3. Markdown artifact 是否保留标题层级与正文顺序
4. Trace 中是否出现 PDF 处理与增强提取相关事件
...
async with client:
    run = await client.call_tool("run_single_tool_validation", {
        ...
        "label": "复杂 PDF Markdown 精查"
    })
```

### 8. 使用 MCP Hub 验证链接发现链路

适用场景：

- 希望检查页面中的链接抽取效果
- 需要联合验证 `extract_links` 与 `extract_structured_data`

推荐场景：`link_discovery_pipeline`

```python
async with client:
    run = await client.call_tool("run_validation_scenario", {
        "scenario_id": "link_discovery_pipeline",
        "inputs": {
