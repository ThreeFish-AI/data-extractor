...
工具层采用领域拆分的模块化设计，所有工具注册于 [`negentropy/perceives/tools/`](../negentropy/perceives/tools/) 子包：

- **[`_registry.py`](../negentropy/perceives/tools/_registry.py)**：中心枢纽，持有 FastMCP `app` 实例、共享服务单例（`web_scraper`、`anti_detection_scraper`、`markdown_converter`）、`create_pdf_processor()` 延迟加载工厂和公共辅助函数
- **领域模块**：各模块导入 `app` 并通过 `@app.tool()` 装饰器注册工具

```mermaid
graph LR
    subgraph "negentropy/perceives/tools/"
...
[`negentropy/perceives/schemas.py`](../negentropy/perceives/schemas.py) 定义 13 个 Pydantic 响应模型（`ScrapeResponse`、`BatchScrapeResponse`、`LinkItem`、`LinksResponse`、`PageInfoResponse`、`RobotsResponse`、`StructuredDataResponse`、`MarkdownResponse`、`BatchMarkdownResponse`、`PDFResponse`、`BatchPDFResponse`、`MetricsResponse`、`CacheOperationResponse`），构成工具层的 API 契约。

### 传输模式

[`server.py`](../negentropy/perceives/server.py) 入口函数 `main()` 支持 **STDIO**（默认）、**HTTP**（可配置 host/port/path/CORS）和 **SSE** 三种传输模式。

## 处理引擎层

### 网页抓取引擎

[`negentropy/perceives/scraper.py`](../negentropy/perceives/scraper.py) 中的 `WebScraper` 作为门面类，持有三个后端实例并通过 `if/elif` 方法分发进行路由：
...
[`negentropy/perceives/anti_detection.py`](../negentropy/perceives/anti_detection.py) 中的 `AntiDetectionScraper` 提供两种隐身后端：
...
[`negentropy/perceives/pdf/`](../negentropy/perceives/pdf/) 子包采用多引擎架构，支持 LLM 智能编排：

- **[`PDFProcessor`](../negentropy/perceives/pdf/processor.py)**：主处理器，支持 `auto`/`pymupdf`/`pypdf`/`docling`/`smart` 五种方法。`auto` 模式优先使用 PyMuPDF，失败时回退到 pypdf。支持 URL 下载、页范围选择、元数据提取
- **[`DoclingEngine`](../negentropy/perceives/pdf/docling_engine.py)**：AI 布局分析引擎，基于 [Docling](https://github.com/DS4SD/docling)，提供 TableFormer 表格结构识别、代码检测和公式提取（可选依赖）
- **[`EnhancedPDFProcessor`](../negentropy/perceives/pdf/enhanced.py)**：增强处理器，提取图像（保存文件 + base64）、识别表格（管道符/制表符/空格分隔模式匹配）、检测 LaTeX 数学公式
- **[`LLMOrchestrator`](../negentropy/perceives/pdf/llm_orchestrator.py)**：LLM 编排中枢（`method="smart"`），三阶段流水线协调多引擎并行处理并择优融合（可选依赖 `litellm`）
- **[`LLMClient`](../negentropy/perceives/pdf/llm_client.py)**：LiteLLM 客户端封装，支持 ZhipuAI GLM-5 等模型
...
[`negentropy/perceives/markdown/converter.py`](../negentropy/perceives/markdown/converter.py) 中的 `MarkdownConverter` 包装 Microsoft [MarkItDown](https://github.com/microsoft/markitdown) 库，附加预处理和后处理流水线：
...
[`negentropy/perceives/form_handler.py`](../negentropy/perceives/form_handler.py) 中的 `FormHandler` 支持 Selenium 和 Playwright 双后端（通过 `hasattr(driver_or_page, "fill")` 检测），处理文本输入、下拉选择、复选框/单选框、文件上传和表单提交。
...
| 组件 | 文件 | 实现方式 |
|------|------|---------|
| `RateLimiter` | [`rate_limiter.py`](../negentropy/perceives/rate_limiter.py) | 时间间隔限速器：记录上次请求时间，间隔不足时 `asyncio.sleep` |
| `RetryManager` | [`retry.py`](../negentropy/perceives/retry.py) | 指数退避重试：`base_delay × backoff_factor^attempt`（全局配置：max=3, factor=2.0） |
| `CacheManager` | [`cache.py`](../negentropy/perceives/cache.py) | 内存 TTL 缓存：MD5 键、可配置过期时间，FIFO 淘汰策略（按时间戳淘汰最旧条目） |
| `MetricsCollector` | [`metrics.py`](../negentropy/perceives/metrics.py) | 计数器/累加器：统计总请求数、成功/失败数、累计耗时、方法使用分布、错误分类 |
| `record_error()` | [`tools/_registry.py`](../negentropy/perceives/tools/_registry.py) | 字符串匹配分类器：按关键字匹配为 timeout/connection/not_found/forbidden/anti_bot/unknown |
...
[`negentropy/perceives/config.py`](../negentropy/perceives/config.py) 基于 `pydantic-settings` 的 `BaseSettings` 实现：
...
| 模块 | 类/函数 | 功能 |
|------|---------|------|
| [`url_utils.py`](../negentropy/perceives/url_utils.py) | `URLValidator` | URL 校验、规范化、域名提取 |
| [`text_utils.py`](../negentropy/perceives/text_utils.py) | `TextCleaner` | 文本清理、邮箱/电话提取、截断 |
| [`config.py`](../negentropy/perceives/config.py) | `ConfigValidator` | 提取配置字典校验（已合并至 config 模块） |
| [`browser_utils.py`](../negentropy/perceives/browser_utils.py) | `build_chrome_options()` | 共享 Chrome 选项构建器 |
